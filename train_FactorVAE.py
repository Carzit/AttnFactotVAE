...
from nets import FactorVAE, AttnFactorVAE
        ...
        for epoch in range(self.max_epoches):
            ...
            with torch.no_grad(): 
                for batch, (quantity_price_feature, fundamental_feature, label) in enumerate(tqdm(self.val_loader, desc=f"Epoch [{epoch+1}/{self.max_epoches}] Val")):
                    ...
                    quantity_price_feature = quantity_price_feature.to(device=self.device)
                    label = label.to(device=self.device)
                    y_hat, mu_posterior, sigma_posterior, mu_prior, sigma_prior = model(quantity_price_feature, 
                                                                                        label)
...
if __name__ == "__main__":
    ...
    model = FactorVAE(quantity_price_feature_size=args.quantity_price_feature_size,
    ...
    encoder_state_dict = {k.removeprefix("encoder."): v for k, v in pretrained_attnvae.state_dict().items() if k.startswith("encoder.")}
    decoder_state_dict = {k.removeprefix("decoder."): v for k, v in pretrained_attnvae.state_dict().items() if k.startswith("decoder.")}
    predictor_state_dict = {k.removeprefix("predictor."): v for k, v in pretrained_attnvae.state_dict().items() if k.startswith("predictor.")}
